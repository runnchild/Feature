 ...
 * @since 2019-05-18
 */
public class AnyEventObserver<T> implements LifecycleObserver, Observer<T> {
    ...
    public AnyEventObserver(LiveData<T> liveData, LifecycleOwner owner, Observer<? super T> observer) {
        mLiveData = liveData;
        mOwner = owner;
        mObserver = observer;
    ...
    public static <T> void bind(LiveData<T> liveData,
                                LifecycleOwner owner,
                                Observer<? super T> observer) {
        if (owner.getLifecycle().getCurrentState() == Lifecycle.State.DESTROYED) {
            return;
        }
        new AnyEventObserver<>(liveData, owner, observer);
    }
}